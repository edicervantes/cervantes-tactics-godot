# Asset Licenses

This repository separates code licensing from asset licensing.

## Code

Source code, Godot scenes, Godot resources, shaders, and project configuration are licensed under the MIT license unless a file says otherwise.

## Original Cervantes Tactics Material

Original Cervantes Tactics characters, logos, portraits, standees, writing, story concepts, generated concept art, and worldbuilding are included so the prototype can run and be reviewed.

These original creative assets are not provided as general-purpose stock assets for reuse outside Cervantes Tactics without permission. See `CONTENT_LICENSE.md` for the narrative, art, visual identity, and audio license terms.

## Third-Party Assets

### Kenney Modular Dungeon Kit

- Source: https://opengameart.org/content/modular-dungeon-kit
- Author: Kenney
- Website: https://kenney.nl
- License: Creative Commons CC0
- Local path: `assets/third_party/kenney_modular_dungeon_kit/`

Attribution is not required by CC0, but crediting Kenney is encouraged.

### KayKit Dungeon Pack

- Source: https://kaylousberg.com/game-assets
- Author: Kay Lousberg
- Website: https://www.kaylousberg.com
- License: Creative Commons CC0
- Local path: `assets/3d/kaykit_dungeon/`

Attribution is not required by CC0, but crediting Kay Lousberg is encouraged.

### Stylized Textures of Shrubs and Tree Tops

- Source: https://opengameart.org/content/stylized-textures-of-shrubs-and-tree-tops
- Author: FabinhoSC
- License: Public Domain / Creative Commons CC0
- Local path: `assets/textures/foliage/`

Attribution is not required by CC0, but crediting FabinhoSC is encouraged.

## Not Included In This Public Mirror

The working project may contain family pitch files, exported media packs, and internal handoff notes. Those are intentionally excluded from this public repository until their public licensing and privacy status is decided.
